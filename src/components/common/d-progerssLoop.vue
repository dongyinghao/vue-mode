<template>
  <div class="percentloop">
    <div class="circle-left" :style="{'backgroundColor': this.bgColor}">
      <div ref="leftcontent"></div>
    </div>
    <div class="circle-right" :style="{'backgroundColor': this.bgColor}">
  ...
  methods: {
    ...
    rotateLeft (deg) { // 大于180时，执行的动画
      this.$nextTick(function () {
        this.$refs.leftcontent && (this.$refs.leftcontent.style.transform = 'rotate(' + (deg - 180) + 'deg)')
      })
    },
    rotateRight (deg) { // 小于180时，执行的动画
      this.$nextTick(function () {
        this.$refs.rightcontent && (this.$refs.rightcontent.style.transform = 'rotate(' + deg + 'deg)')
      })
    },
    goRotate (deg) {
